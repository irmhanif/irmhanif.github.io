 ...
 * @since  1.6
 */
class Fixed_tripModelCreate_planningForm extends JModelForm
{
    private $item = null;
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_fixed_trip.create_planning', 'create_planningform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_fixed_trip.edit.create_planning.data', array());
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('create_planning.id');
    ...
    public function delete($pk)
    {
        $user = Factory::getUser();
            ...
            if (empty($pk))
            {
                    $pk = (int) $this->getState('create_planning.id');
            }

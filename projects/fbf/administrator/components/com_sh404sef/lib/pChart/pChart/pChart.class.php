...
class pChart
{
  ...
  function clearScale()
  {
    ...
    function drawBasicPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$R=255,$G=255,$B=255,$Decimals=0)
    {
      ...
      {
	    $TopPlots[$Key] = !empty($TopPlots[$Key]) && is_array($TopPlots[$Key]) ? $TopPlots[$Key] : array();
        $TopPlots[$Key][] = $XPos;
        $TopPlots[$Key][] = $YPos;
    ...
    function drawPieGraph($Data,$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$EnhanceColors=TRUE,$Skew=60,$SpliceHeight=20,$SpliceDistance=0,$Decimals=0)
    {
      ...
      foreach($iValues as $Key => $Value)
      {
	    ...
	    $TopPlots[$Key] = !empty($TopPlots[$Key]) && is_array($TopPlots[$Key]) ? $TopPlots[$Key] : array();
	      $BotPlots[$Key] = !empty($BotPlots[$Key]) && is_array($BotPlots[$Key]) ? $BotPlots[$Key] : array();
	      $aTopPlots[$Key] = !empty($aTopPlots[$Key]) && is_array($aTopPlots[$Key]) ? $aTopPlots[$Key] : array();
	      $aBotPlots[$Key] = !empty($aBotPlots[$Key]) && is_array($aBotPlots[$Key]) ? $aBotPlots[$Key] : array();
    ...
    {
	  $this->Errors = is_array($this->Errors) ? $this->Errors : array();
    ...
    function addToImageMap($X1,$Y1,$X2,$Y2,$SerieName,$Value,$CallerFunction)
    {
      ...
      {
	    $this->ImageMap = is_array($this->ImageMap) ? $this->ImageMap : array();

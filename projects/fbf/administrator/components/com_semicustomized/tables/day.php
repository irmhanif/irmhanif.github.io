 ...
 * @since  1.6
 */
class SemicustomizedTableday extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'SemicustomizedTableday', array('typeAlias' => 'com_semicustomized.day'));
		parent::__construct('#__semicustomized_days', 'id', $db);
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_semicustomized.day.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_semicustomized/access.xml',
				"/access/section[@name='day']/"
			);
			$default_actions = JAccess::getAssetRules('com_semicustomized.day.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['picture'][0]['size'] > 0)
		{
			...
			$oldFiles = SemicustomizedHelper::getFiles($this->id, $this->_tbl, 'picture');
	...
	protected function _getAssetName()
	{
		...
		return 'com_semicustomized.day.' . (int) $this->$k;
	}
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
				JFile::delete(JPATH_ROOT . '/uploads/' . $pictureFile);
			}
		}

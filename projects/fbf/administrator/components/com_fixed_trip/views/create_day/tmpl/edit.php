...
<script type="text/javascript">
	...
	Joomla.submitbutton = function (task) {
		if (task == 'create_day.cancel') {
			Joomla.submitform(task, document.getElementById('create_day-form'));
		}
		else {
			
			if (task != 'create_day.cancel' && document.formvalidator.isValid(document.id('create_day-form'))) {
...
<form
	...
	method="post" enctype="multipart/form-data" name="adminForm" id="create_day-form" class="form-validate">

	<div class="form-horizontal">
		<?php echo JHtml::_('bootstrap.startTabSet', 'myTab', array('active' => 'general')); ?>

		<?php echo JHtml::_('bootstrap.addTab', 'myTab', 'general', JText::_('COM_FIXED_TRIP_TITLE_CREATE_DAY', true)); ?>
		<div class="row-fluid">
				...
				<?php if (!empty($this->item->picture)) : ?>
					...
					<?php foreach ((array)$this->item->picture as $fileSingle) : ?>
						<?php if (!is_array($fileSingle)) : ?>
							<a href="<?php echo JRoute::_(JUri::root() . 'create_day' . DIRECTORY_SEPARATOR . $fileSingle, false);?>"><?php echo $fileSingle; ?></a> | 

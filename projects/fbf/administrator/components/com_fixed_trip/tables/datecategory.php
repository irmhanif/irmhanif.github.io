...
/**
 * datecategory Table class
 *
 * @since  1.6
 */
class Fixed_tripTabledatecategory extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'Fixed_tripTabledatecategory', array('typeAlias' => 'com_fixed_trip.datecategory'));
		parent::__construct('#__date_category', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_fixed_trip.datecategory.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_fixed_trip/access.xml',
				"/access/section[@name='datecategory']/"
			);
			$default_actions = JAccess::getAssetRules('com_fixed_trip.datecategory.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['picture'][0]['size'] > 0)
		{
			...
			{
				$oldFile = JPATH_ROOT . '/date_category/' . $f;
			...
			foreach ($files['picture'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '/date_category/' . $filename;
	...
	protected function _getAssetName()
	{
		...
		return 'com_fixed_trip.datecategory.' . (int) $this->$k;
	}
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
				JFile::delete(JPATH_ROOT . '/date_category/' . $pictureFile);
			}
		}

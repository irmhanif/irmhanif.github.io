...
/**
 * create_day Table class
 *
 * @since  1.6
 */
class Fixed_tripTablecreate_day extends JTable
{
	...
	{
		JObserverMapper::addObserverClassToClass('JTableObserverContenthistory', 'Fixed_tripTablecreate_day', array('typeAlias' => 'com_fixed_trip.create_day'));
		parent::__construct('#__fixed_trip_days', 'id', $db);
	}
	...
	public function bind($array, $ignore = '')
	{
		...
		if (!JFactory::getUser()->authorise('core.admin', 'com_fixed_trip.create_day.' . $array['id']))
		{
			$actions         = JAccess::getActionsFromFile(
				JPATH_ADMINISTRATOR . '/components/com_fixed_trip/access.xml',
				"/access/section[@name='create_day']/"
			);
			$default_actions = JAccess::getAssetRules('com_fixed_trip.create_day.' . $array['id'])->getData();
			$array_jaccess   = array();
	...
	public function check()
	{
		...
		if ($files['picture'][0]['size'] > 0)
		{
			...
			{
				$oldFile = JPATH_ROOT . '/create_day/' . $f;
			...
			foreach ($files['picture'] as $singleFile )
			{
				...
				else
				{
					...
					$uploadPath = JPATH_ROOT . '/create_day/' . $filename;
	...
	protected function _getAssetName()
	{
		...
		return 'com_fixed_trip.create_day.' . (int) $this->$k;
	}
	...
	public function delete($pk = null)
	{
		...
		if ($result)
		{
			...
			{
				JFile::delete(JPATH_ROOT . '/create_day/' . $pictureFile);
			}
		}
